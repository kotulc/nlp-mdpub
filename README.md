...
### cli
The command line interface package contains all modules that define the CLI app, its available commands, options, flags, and interface to the `core` logic and `crud` layers. 
...
### crud
The crud package is the data persistance layer that defines the interface surface between the database (stored data, table methods, utilties) and the rest of the application.
...
- Try to keep lines to 100 columns/characters or less when it makes sense
...
- Include a description of each module in a short comment block at the top of the module (before imports)

#### Dont's
- DO NOT vertically space arguments or lines of code unless completely necessary.
